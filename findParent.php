...
$stmt = $con->prepare("SELECT pid, fullName, sex, birthLikeDate, birthLikePlaceText FROM ancestors JOIN Parents ON ancestor.pid = Parents.father_id
  WHERE Parents.child_id = ? UNION SELECT pid, fullName, sex, birthLikeDate, birthLikePlaceText FROM ancestors JOIN Parents ON ancestor.pid = Parents.mother_id
  WHERE Parents.child_id = ?");
$stmt->bind_param('sss', $input_id);
$stmt->execute();
